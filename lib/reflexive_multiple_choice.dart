...
class ReflexiveMultipleChoice extends StatefulWidget {
  final void Function() onNextQuestion; // Callback function

  ReflexiveMultipleChoice({required this.onNextQuestion});

  @override
  _ReflexiveMultipleChoiceState createState() => _ReflexiveMultipleChoiceState();
}

class _ReflexiveMultipleChoiceState extends State<ReflexiveMultipleChoice> {
  List<List<dynamic>>? relationsList;
  ...
  void fetchQuestion() async {
    ...
    final response = await http.get(Uri.parse('http://127.0.0.1:5000/quiz?question_type=7'));
    if (response.statusCode == 200) {
      setState(() {
        // Update state with new question data
        Map<String, dynamic> responseData = json.decode(response.body);
        relationsList = List<List<dynamic>>.from(responseData['relations']);
        correctRelationIndex = responseData['reflexive_index'];
      ...
      body: relationsList == null
          ...
          : Padding(
              ...
              child: Column(
                ...
                children: [
                  Text('Which of the given Relations on the Set {1, 2, 3, 4, 5, 6} is Reflexive:'),
                  SizedBox(height: 16),
                      ...
                      (index) => Column(
                        children: [
                          ElevatedButton(
                            ...
                            child: Text('Relation ${index + 1}: ${_convertToSet(relationsList![index])}'),
                            style: ButtonStyle(
