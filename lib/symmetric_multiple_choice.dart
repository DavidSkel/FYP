...
class SymmetricMultipleChoice extends StatefulWidget {
  final void Function() onNextQuestion; // Callback function

  SymmetricMultipleChoice({required this.onNextQuestion});

  @override
  _SymmetricMultipleChoiceState createState() => _SymmetricMultipleChoiceState();
}

class _SymmetricMultipleChoiceState extends State<SymmetricMultipleChoice> {
  ...
  void fetchQuestion() async {
    ...
    final response = await http.get(Uri.parse('http://127.0.0.1:5000/quiz?question_type=5'));
    if (response.statusCode == 200) {
      setState(() {
        // Update state with new question data
        Map<String, dynamic> responseData = json.decode(response.body);
        relationsList = List<List<dynamic>>.from(responseData['relations']);
        correctRelationIndex = responseData['symmetric_index'];
    ...
    return Scaffold(
      ...
      body: relationsList == null
        ...
        child: Column(
          ...
          children: [
            Text('Which of the given Relations on the Set {1, 2, 3, 4, 5, 6} is Symmetric:'),
            SizedBox(height: 16),
